...
ssh root@${NODE_HOST} "mkdir -p ${DEPLOY_FOLDER}/chef-repo"
...
if [ ${COPY_ARTIFACTS} == "true" ]; then
    # Clear RPM repo
    ssh root@${NODE_HOST} "rm -rf ${RPM_REPO_ROOT}/rpm-repo/*.rpm"

	# Create target rpm repo
	ssh root@${NODE_HOST} "mkdir -p ${RPM_REPO_ROOT}"

	# Copy RPMs to RPM repo
	scp -rv "${WORKSPACE}/boxes/target/shared/rpm-repo" root@${NODE_HOST}:${RPM_REPO_ROOT}
fi
...
# Copy licenses to node
scp -rv "${tmp_dir}/licenses" root@${NODE_HOST}:${CM_CONFIG}

# Clean up
rm -rf ${tmp_dir}

# Provision the node
ssh root@${NODE_HOST} "${DEPLOY_FOLDER}/scripts/provision-node.sh '${DEPLOY_FOLDER}' ${NODE_HOST} ${ENVIRONMENT}"
